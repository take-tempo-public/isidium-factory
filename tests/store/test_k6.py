...
def test_the_journal_row_records_the_credential_and_the_version(svc: tuple[Service, Harness]) -> None:
    ...
    assert hz.st.config_tree["schema"] == REGISTRY.newest("config") == 7 and cfg.journal_schema(hz.st.eff) == 2
...
def test_a_config1_tenant_writes_v1_rows_until_the_policy_act_moves_it(tmp_path: Path) -> None:
    ...
    v1 = Store(
        ...
        registry_without(tmp_path, "config@2", "config@3", "config@4", "config@5", "config@6", "config@7"),
        clock,
        signer,
        root="docs/work/",
    )
...
def test_the_cli_hands_the_owners_file_over_with_its_own_head_as_the_base(tmp_path: Path) -> None:
    ...
    assert args["document"]["schema"] == REGISTRY.newest("config") == 7 and args["document"]["tenant"] == "k6-cli"
...
def test_the_journal_key_is_config2s_and_a_version_this_store_cannot_write_is_refused() -> None:
    ...
    # `config@7` since V4a-ii-a; `config@6` since V4a-i; `config@5` since V3; `config@4` since L4; `config@3` since K10; still journal@2
    assert REGISTRY.newest("config") == 7 and REGISTRY.newest("journal") == 2
