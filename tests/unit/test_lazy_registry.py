...
def test_opening_a_registry_reads_nothing_and_installed_costs_no_parse(schemas: Path) -> None:
    ...
    Both halves matter: the count is 0, **and** the refs that come back are the seventeen the directory holds
    (eight before K6 added `config@2`, `journal@1` and `journal@2`; eleven before K10 added `config@3`; twelve
    before L4 added `config@4`, `sidecar@2` and `sidecar-events@2`; fifteen before V3 added `config@5` and
    `sidecar-events@3`). A registry that had quietly stopped finding
    documents would satisfy the count on its own."""
    ...
    assert refs == frozenset(on_disk) == held and len(refs) == 19, sorted(refs)
...
def test_the_shipped_registry_is_opened_the_same_way() -> None:
    ...
    assert {"registry@1", "config@1", "card@1", "config@7", "sidecar-events@3"} <= refs and len(refs) == 19, sorted(
        refs
    )
...
def test_a_toml_the_registry_cannot_name_is_refused_when_the_directory_is_listed(schemas: Path) -> None:
    ...
    assert (schemas / "INSTALLED").is_file() and len(Registry.from_directory(schemas).installed) == 19
