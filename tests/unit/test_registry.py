...
def test_d1_fixtures_validate() -> None:
    ...
    # the version that does not ship: `config@8` since V4a-ii-a shipped `config@7` (the arm moves with each)
    unshipped = text.replace("schema = 2", "schema = 8", 1).replace('schema = "config@2"', 'schema = "config@8"', 1)
