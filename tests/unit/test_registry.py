...
def test_d1_fixtures_validate() -> None:
    ...
    # the version that does not ship: `config@7` since V4a-i shipped `config@6` (the arm moves with each)
    unshipped = text.replace("schema = 2", "schema = 7", 1).replace('schema = "config@2"', 'schema = "config@7"', 1)
