...
def open_intake_ids(inbox: Sequence[Mapping[str, Any]]) -> frozenset[Any]:
    """Which intake records are still open (03a): every `intake` id, minus the ones a disposition has closed —
    `accepted` or `declined` only (R2: `deferred`, and anything else, leaves the id open). The one answer `queue`'s
    `inbox_counts` and `board.render`'s `## Inbox` list both read, so the two cannot disagree on it (card 9, R1)."""
