...
| **execution status** (line state) | `dispatched` · `parked` · `answered` · `failed(class)` with `class ∈ {scope, ambiguity, budget, timeout, infra, environment, acceptance, gate, card-drift, identity}` (the last six added [owner, 2026-09-12, Q-V22 (a)]: the classes T-A9's and T-C6's failure protocols and the ledger already use, so a run whose `dispatched` landed can land its end) · `complete` · `closed` · `reverted` · `abandoned` (a run in flight when the card was withdrawn or demoted, card 7) · `disputed` · `reopened` | `state.json` (from the ledger), `lander` only |
